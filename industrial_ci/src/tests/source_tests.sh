...
function run_source_tests {
    ...
    if [ "$CCACHE_DIR" ]; then
        ...
        UPSTREAM_CMAKE_ARGS="$UPSTREAM_CMAKE_ARGS -DCMAKE_CXX_COMPILER_LAUNCHER=/usr/bin/ccache -DCMAKE_C_COMPILER_LAUNCHER=/usr/bin/ccache"
        TARGET_CMAKE_ARGS="$TARGET_CMAKE_ARGS -DCMAKE_CXX_COMPILER_LAUNCHER=/usr/bin/ccache -DCMAKE_C_COMPILER_LAUNCHER=/usr/bin/ccache"
        DOWNSTREAM_CMAKE_ARGS="$DOWNSTREAM_CMAKE_ARGS -DCMAKE_CXX_COMPILER_LAUNCHER=/usr/bin/ccache -DCMAKE_C_COMPILER_LAUNCHER=/usr/bin/ccache"
    fi
